...
<head>
    ...
    <style>
        ...
        /* Mejora para navegación por teclado (focus visible) */
        *:focus-visible {
...
<body class="bg-[#12141c] text-white flex flex-col items-center justify-center min-h-screen p-4">
    ...
    <main class="w-full max-w-sm flex flex-col items-center">
        ...
        <div class="w-full border border-gray-700 rounded-2xl p-6 bg-transparent">
            ...
            @if ($errors->any())
                <div class="mb-4 text-sm text-red-400 text-center bg-red-900/30 p-2 rounded" role="alert">
            ...
            <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-4">
                ...
                <div class="text-center mt-2">
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-xs text-gray-400 hover:text-white transition underline-offset-2 hover:underline focus:text-white">
        ...
        <div class="w-full flex gap-4 mt-8">
            <button class="flex-1 bg-white text-black font-semibold rounded-xl py-3 flex items-center justify-center gap-2 hover:bg-gray-200 transition text-sm focus:ring-4 focus:ring-white/50">
                <span class="text-xl" aria-hidden="true"></span> Apple
            </button>

            <button class="flex-1 bg-white text-black font-semibold rounded-xl py-3 flex items-center justify-center gap-2 hover:bg-gray-200 transition text-sm focus:ring-4 focus:ring-white/50">
                <span class="text-lg" aria-hidden="true">G</span> Google
            </button>
        </div>
    </main>
</body>
